 ...
 * @subpackage Fh_Download_Pdf/public
 */
...
class Fh_Download_Pdf_Public
{
	...
	public function enqueue_styles()
	{
		...
		wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/fh-download-pdf-public.css', array(), $this->version, 'all');
	}
	...
	public function enqueue_scripts()
	{
		...
		wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/fh-download-pdf-public.js', array('jquery'), $this->version, true);
	}

	/**
	 * Example of Shortcode processing function.
	 *
	 * Shortcode can take attributes like [fh-download-pdf-shortcode attribute='123']
	 * Shortcodes can be enclosing content [fh-download-pdf-shortcode attribute='123']custom content[/fh-download-pdf-shortcode].
	 *
	 ...
	 * @return void
	 */
	public function shortcode_download_pdf_btn($atts, $content = null, $tag)
	{
		/**
		 * Combine user attributes with known attributes.
		 *
		...
		if (intval($atts['published']) == 1) {
			...
			include(__DIR__ . '/partials/fh-download-pdf-public-display.php');
			return $output;
		}
	}
	...
	{
		return get_option('fh_download_pdf_settings')['select_field'];
	}
	...
	{
		return get_option('fh_download_pdf_settings')['name_textfield'];
	}
